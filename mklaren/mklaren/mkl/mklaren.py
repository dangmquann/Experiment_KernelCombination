...
class Mklaren:
    ...
    def fit(self, Ks, y):
        ...
        for pi in range(no_kernels):
            ...
            data[pi]["ina"]  = list(range(n))             # inactive set                #FIXME: check if this is correct (range(n))
        ...
        for pi in range(no_kernels):
            ...
            data[pi]["ina"] = list(range(n))  # Ensure ina is a list
            data[pi]["k"]         = 0                       # column count
        ...
        for xk in range(rank):
            ...
            ina = list(ina) if isinstance(ina, ndarray) else ina
    ...
    #     # Ensure ina is a list
    #     ina = list(ina) if isinstance(ina, ndarray) else ina    #FIXME: check if this is correct
    ...
    def cholesky_steps(self, K, G, D, start, act, ina, order=None, no_steps=None):
        if order is None:
            no_steps = K.shape[0] if no_steps is None else no_steps
            have_order = False
        else:
        ...
        ina = list(ina) if isinstance(ina, np.ndarray) else ina
        ...
        for ki, k in enumerate(range(start, start + no_steps)):
            ...
            if self.debug:
                norm_diff_before = norm(K[:, :] - G[:, :k].dot(G[:, :k].T))
                norm_diff_after = norm(K[:, :] - G[:, :k+1].dot(G[:, :k+1].T))
                print(f"Step {k}: norm_diff_before = {norm_diff_before}, norm_diff_after = {norm_diff_after}")
                if norm_diff_before < norm_diff_after:
                    print(f"Assertion failed at step {k}")
                    print(f"i: {i}, j: {j}")
                    print(f"G[i, k]: {G[i, k]}")
                    print(f"G[j, k]: {G[j, k]}")
                    print(f"D[j]: {D[j]}")
                    print(f"D[i]: {D[i]}")
                assert norm_diff_before >= norm_diff_after
